...
def index():
    ...
    btableh = THEAD(TR(TH(T("Customer")),TH(T("Proxy")),TH(T("PAC-File")),TH(T("Status")),TH(T("Entries")),TH(T("addedat/addedby")),TH(T("Edit"),TH(T("Delete")))))
    ...
    for item in db(db.auth_user.id==db.customers.addedby).select():
        ...
        btableb.append(TR(TD(item.customers.customer),TD(item.customers.proxyurl+":"+str(item.customers.proxyport)),TD(item.customers.pacfile),TD(status),TD(A(entrycount,_href=URL("default","multiChangesCustomersEdit",args=item.customers.id))),TD(str(item.customers.addedat.strftime("%d%b%Y"))+"/"+str(item.auth_user.username)),TD(A(SPAN(_class="glyphicon glyphicon-cog"),_href=URL("editCustomer",args=item.customers.id))),TD(A(SPAN(_class="glyphicon glyphicon-trash"),_href=URL("delCustomer",args=item.customers.id)))))
...
def addCustomer():
    ...
    div0.append(INPUT(_name="customername",requires = IS_NOT_IN_DB(db,db.customers.customer,error_message=T("Customer name must be unique and not empty")),_class="form-control"))
    ...
    div3.append(INPUT(_name="pacfile",_class="form-control", requires = [IS_NOT_IN_DB(db,db.customers.pacfile),IS_MATCH(".*\.php$",error_message=T("PAC-File name must end with .php"))]))
    ...
    if form.accepts(request,session,keepvalues=True):
        try:
            db.customers.insert(customer=form.vars.customername,proxyurl=form.vars.proxyfqdn,proxyport=form.vars.proxyport,pacfile=form.vars.pacfile,addedby=auth.user.id,addedat=datetime.datetime.now(),isActive=form.vars.isActive)
        except:
            response.flash =  sys.exc_info()[1]
        else:
...
def editCustomer():
    ...
    div1 = DIV(_class="form-group")
    div1.append(LABEL(T("Proxy-FQDN (e.g. proxy.r-services.at)")))
    div1.append(INPUT(_name="proxyurl",_value=customer.proxyurl,_class="form-control", requires = IS_URL(prepend_scheme=None)))
    div2 = DIV(_class="form-group")
    div2.append(LABEL(T("Proxy-Port (e.g. 57145)")))
    div2.append(INPUT(_name="proxyport", _value=customer.proxyport, _class="form-control", requires = IS_INT_IN_RANGE(1,65536)))
    div3 = DIV(_class="form-group")
    div3.append(LABEL(T("PAC-File (e.g. mwg_riz.php)")))
    div3.append(INPUT(_name="pacfile",_value=customer.pacfile, _class="form-control", requires = [IS_NOT_IN_DB(db,db.customers.pacfile),IS_MATCH(".*\.php$",error_message=T("PAC-File name must end with .php"))]))
    div4 = DIV(_class="form-group")
    div4.append(LABEL(T("is active?"),INPUT(_name="isActive",_class="form-control",_type="checkbox",value=customer.isActive)))
    ...
    if form.accepts(request,session,keepvalues=True):
        try:
            db(db.customers.id==request.args[0]).update(proxyurl = form.vars.proxyurl, proxyport = form.vars.proxyport, pacfile = form.vars.pacfile, isActive = form.vars.isActive)
        except:
            response.flash =  sys.exc_info()[1]
        else:
