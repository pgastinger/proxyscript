...
def generate(id):
    ...
    if customer:
        entries = db((db.proxybypassentries.customer==id)&(db.proxybypassentries.customer==db.customers.id)&(db.proxybypassentries.destination==db.destinations.id)&(db.auth_user.id==db.proxybypassentries.addedby)).select()
        content = ""
        for entry in entries:
            m = IP_CIDR_RE.search(str(entry.destinations.destination))
            if m:
                n = IPy.IP(entry.destinations.destination)
                content+=entrytemplate_ip%(entry.proxybypassentries.addedat.strftime("%d%b%Y"),entry.auth_user.username,entry.proxybypassentries.info,n.net(),n.netmask())
            else:
                content+=entrytemplate_domain%(entry.proxybypassentries.addedat.strftime("%d%b%Y"),entry.auth_user.username,entry.proxybypassentries.info,entry.destinations.destination)
        custom_entries = db((db.customentries.customer==id)&(db.customentries.isActive==True)).select()
        for item in custom_entries:
            content += "\n"
            content+="\t//%s/%s %s\n"%(entry.proxybypassentries.addedat.strftime("%d%b%Y"),entry.auth_user.username,entry.proxybypassentries.info)
            content+=item.entry
            content += "\n"
...
def pacTester():
    ...
    div0.append(SELECT(_name="pacfileid",*[OPTION(item.pacfile,_value=str(item.id)) for item in db(db.customers.isActive==True).select()],_class="form-control", requires = IS_IN_DB(db,'customers.id','%(pacfile)s')))
...
def showFile():
    ...
    if len(request.args) != 1:
        redirect(URL("index"))
    if IS_INT_IN_RANGE(1, 100)(request.args[0])[1] != None:
        redirect(URL("index"))
...
def multiChangesDestinations():
    ...
    div0.append(SELECT(_name="destination",*[OPTION(item.destination,_value=str(item.id)) for item in db(db.destinations.isActive==True).select()],_class="form-control select2", requires = IS_IN_DB(db,'destinations.id','%(destination)s')))
    form.append(div0)
...
def multiChangesDestinationsEdit():
    ...
    customers = db(db.customers.isActive==True).select()
    entries = db((db.proxybypassentries.destination==request.args[0])&(db.customers.id==db.proxybypassentries.customer)&(db.destinations.id==request.args[0])).select()
...
# remove entries, which are not checked any more from database for customer
        for item in entries:
            ...
            if form.vars.has_key(key) and form.vars[key] is None:
                logger.info("Remove Destination %s for customer %s by %s with info %s"%(destination.destination,item.customers.customer,auth.user.username,info))
                db((db.proxybypassentries.destination==destination.id)&(db.proxybypassentries.customer==item.customers.id)).delete()
        ...
        for item in form.vars:
            if item.startswith("customer") and form.vars[item]:
                ...
                if len(db((db.proxybypassentries.customer==cid)&(db.proxybypassentries.destination==destination.id)).select()) == 0 :
                    logger.info("Adding Destination %s for customer %s by %s with info %s"%(destination.destination,cid,auth.user.username,info))
                    db.proxybypassentries.insert(addedby=auth.user.id,addedat=datetime.datetime.now(),info=info,customer=cid,destination=destination.id)
...
def multiChangesCustomers():
    ...
    div0.append(SELECT(_name="customer",*[OPTION(item.customer,_value=str(item.id)) for item in db(db.customers.isActive==True).select()],_class="form-control", requires = IS_IN_DB(db,'customers.id','%(customer)s')))
    form.append(div0)
...
def multiChangesCustomersEdit():
    ...
    div0 = DIV(_class="form-group")
    customer = db(db.customers.id==request.args[0]).select().first()
    destinations = db(db.destinations.isActive==True).select()
    entries = db((db.proxybypassentries.customer==request.args[0])&(db.destinations.id==db.proxybypassentries.destination)&(db.customers.id==request.args[0])).select()
...
# remove entries, which are not checked any more from database for customer
        for item in entries:
            ...
            if form.vars.has_key(key) and form.vars[key] is None:
                logger.info("Remove customer %s for destination %s by %s with info %s"%(customer.customer,item.destinations.destination,auth.user.username,info))
                db((db.proxybypassentries.customer==customer.id)&(db.proxybypassentries.destination==item.destinations.id)).delete()

        for item in form.vars:
            if item.startswith("destination") and form.vars[item]:
                # add entries if they are not yet in the database
                did = item.replace("destination","")
                if len(db((db.proxybypassentries.destination==did)&(db.proxybypassentries.customer==customer.id)).select()) == 0 : 
                    logger.info("Adding customer %s for destination %s by %s with info %s"%(customer.customer,did,auth.user.username,info))
                    db.proxybypassentries.insert(addedby=auth.user.id,addedat=datetime.datetime.now(),info=info,customer=customer.id,destination=did)
