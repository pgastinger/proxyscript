...
def index():
    ...
    btableh = THEAD(TR(TH(T("Customer")),TH(T("Custom-Entry")),TH(T("Info")),TH(T("Status")),TH(T("addedat/addedby")),TH(T("Edit"),TH(T("Delete")))))
    btable.append(btableh)
    btableb = TBODY()

    for item in db((db.auth_user.id==db.customentries.addedby)&(db.customers.id==db.customentries.customer)).select():
        ...
        btableb.append(TR(TD(item.customers.customer),TD(PRE(item.customentries.entry)),TD(item.customentries.info),TD(status),TD(str(item.customentries.addedat.strftime("%d%b%Y"))+"/"+str(item.auth_user.username)),TD(A(SPAN(_class="glyphicon glyphicon-cog"),_href=URL("editCustomEntry",args=item.customentries.id))),TD(A(SPAN(_class="glyphicon glyphicon-trash"),_href=URL("delCustomEntry",args=item.customentries.id)))))
...
def addCustomEntry():
    ...
    if form.accepts(request,session,keepvalues=True):
        try:
            id = db.customentries.insert(customer=form.vars.customer,info=form.vars.info,isActive=form.vars.isActive,entry=form.vars.customentry,addedby=auth.user.id,addedat=datetime.datetime.now())
        except:
            response.flash =  sys.exc_info()[1]
        else:
...
def editCustomEntry():
    ...
    if IS_INT_IN_RANGE(1, 1000)(request.args[0])[1] != None:
        redirect(URL("index"))
    customentry = db((db.customers.id==db.customentries.customer)&(db.customentries.id==request.args[0])).select().first()
    form = FORM(_role="form")
    div0 = DIV(_class="form-group")
    div0.append(LABEL(T("Customer")))
    div0.append(INPUT(_name="destinationname",_value=customentry.customers.customer,requires = IS_NOT_EMPTY(),_class="form-control",_readonly="readonly"))
    div1 = DIV(_class="form-group")
    div1.append(LABEL(T("Entry (Valid JavaScript-code necessary)")))
    div1.append(TEXTAREA(customentry.customentries.entry,_name="entry",_class="form-control",requires = IS_NOT_EMPTY()))
    div2 = DIV(_class="form-group")
    div2.append(LABEL(T("Info")))
    div2.append(INPUT(_name="info",_value=customentry.customentries.info,_class="form-control"))
    div3 = DIV(_class="form-group")
    div3.append(LABEL(T("is active?"),INPUT(_name="isActive",_class="form-control",_type="checkbox", value=customentry.customentries.isActive)))
    ...
    if form.accepts(request,session,keepvalues=True):
        try:
            db(db.customentries.id==request.args[0]).update(info = form.vars.info, isActive=form.vars.isActive, entry=form.vars.entry,addedby=auth.user.id,addedat=datetime.datetime.now())
        except:
            response.flash =  sys.exc_info()[1]
        else:
